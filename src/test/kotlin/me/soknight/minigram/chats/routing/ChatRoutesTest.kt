...
class ChatRoutesTest {
    ...
    @Test fun `GET chats returns 200`() = testApplication {
        application { module() }
    ...
    @Test fun `GET chats returns 401 without token`() = testApplication {
        application { module() }
        assertEquals(HttpStatusCode.Unauthorized, client.get("/api/v1/chats").status)
    }

    @Test fun `POST chats creates SAVED chat and returns 201`() = testApplication {
        application { module() }
        val client = createClient { install(ContentNegotiation) { json(appJson) } }
