...
fun Route.chatRoutes(chatService: ChatService, clientFactory: (String) -> ProfileClient) {
    authenticate("jwt") {
        route("/api/v1/chats") {
            get {
                ...
                call.respond(chatService.getChats(userId, clientFactory(call.bearerToken()), page, size))
            }
            post {
                val dto = chatService.createChat(
                    call.receive<CreateChatRequest>(),
                    call.currentUserId(),
                    clientFactory(call.bearerToken())
                )
                call.respond(HttpStatusCode.Created, dto)
            }
            route("/{chat_id}") {
                get {
                    call.respond(chatService.getChat(call.chatId(), call.currentUserId(), clientFactory(call.bearerToken())))
                }
                patch {
                    call.respond(chatService.editChat(call.chatId(), call.receive<EditChatRequest>(), call.currentUserId(), clientFactory(call.bearerToken())))
                }
                put {
                    call.respond(chatService.editChat(call.chatId(), call.receive<EditChatRequest>(), call.currentUserId(), clientFactory(call.bearerToken())))
                }
